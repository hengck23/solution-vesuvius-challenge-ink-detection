...
#
#
# class pvt_v2_b4_level5(PyramidVisionTransformerV2):
#     def __init__(self, **kwargs):
#         super(pvt_v2_b4_level5, self).__init__(
#             patch_size=4,
#             num_stages=5,
...
if 0:
    net = pvt_v2_b0(pretrained=True) #1024
    # print(net)
    x = torch.rand(2, 3, 256, 256)
    out = net(x)
    print([f.shape for f in out])

    checkpoint = '/home/titanx/hengck/share1/data/pretrain_model/' + net.pretrain
    checkpoint = torch.load(checkpoint, map_location=lambda storage, loc: storage)
    # print(list(checkpoint.keys()))
    # ['epoch', 'arch', 'state_dict', 'optimizer', 'version', 'args', 'amp', 'metric']
    state_dict = checkpoint
    print(net.load_state_dict(state_dict, strict=True))
