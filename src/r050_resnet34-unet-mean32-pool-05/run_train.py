...
# from config_fold1_stage1_2 import *
...
def infer_one(net, d):
	...
	start_timer = timer()
	batch_iter = np.array_split(xy, len(xy)//16)
...
def run_train():
	...
	#num_iteration = (num_epoch+0.01)*len(train_loader)
	...
	## start training here! ##############################################
	# bce, threshold , recall, fpr, score
		...
		for t, batch in enumerate(train_loader):
			...
			# if t % 500 == 0:
				...
				for b in range(B):
					t = label[b, 0]
					p = ink[b, 0]
					image_show_norm('t', t, min=0, max=1, resize=0.25)
					image_show_norm('p', p, min=0, max=1, resize=0.25)
					...
					image_show_norm('overlay', overlay, resize=0.25)
