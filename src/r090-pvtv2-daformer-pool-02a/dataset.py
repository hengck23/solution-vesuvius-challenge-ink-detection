from common import *
from ok.dataset.dataset import *
from ok.dataset.augmentation import *
...
def train_augment_v2f(d, cfg):
	...
	if np.random.rand() < 0.2:
		volume = do_random_noise(volume, n=[-0.08, 0.08])
	if np.random.rand() < 0.2:
		volume = do_random_blur(volume, k=[3, 11])
...
class InkDataset(Dataset):
	...
	def __getitem__(self, index):
		...
		r['valid'] = torch.from_numpy(mask).float()
		return r


tensor_key = ['volume', 'label', 'valid',  ]
def null_collate(batch):
	...
	d['valid'] = torch.stack(d['valid']).unsqueeze(1)
	return d
...
def run_check_dataset():
	...
	dataset = InkDataset(train_data, augment=train_augment_v2f, cfg=cfg)
	print(dataset)
